...
def save_npy_as_ply(filename, args):

    import pymesh

    verts=np.load(filename[:-4]+"_predcoords.npy")
    feats=np.load(filename[:-4]+'predfeatures_emb1.npy')
    tri = pymesh.triangle()
    tri.points = vertices
    tri.max_area = 0.05
    ...
    for feat in range(0,args.in_channels):
        mesh.add_attribute("input_feature_"+str(feat))
        mesh.set_attribute("input_feature_"+str(feat), feats[:,feat])
    for feat in range(0,args.emb_dims):
        mesh.add_attribute("emb_feature_"+str(feat))
        mesh.set_attribute("emb_feature_"+str(feat), feats[:,args.in_channels+feat])

    nmap=['0','A','G','C','T']
    assert len(nmap)==args.n_outputs

    for feat in range(0,args.n_outputs):
        mesh.add_attribute("pred_"+nmap[feat])
        mesh.set_attribute("pred_"+nmap[feat], feats[:,args.in_channels+args.emb_dims+args.n_outputs+feat])
    for feat in range(0,args.n_outputs):
        mesh.add_attribute("real_"+nmap[feat])
        mesh.set_attribute("real_"+nmap[feat], feats[:,args.in_channels+args.emb_dims+args.n_outputs+feat])

    mesh.add_attribute("pred_na")
    mesh.set_attribute("pred_na",np.argmax(feats[:,-2*args.n_outputs:-args.n_outputs],axis=1))
    mesh.add_attribute("real_na")
    mesh.set_attribute("real_na",np.argmax(feats[:,-args.n_outputs:],axis=1))
...
def load_ply(
    ...
    try:
        ...
        mesh=save_npy_as_ply(filename, args)
